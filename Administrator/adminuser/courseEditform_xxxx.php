...
$SemforChange = $_POST["courseSemforChange"]; 
...
if($qryresult) {
 ...
 $recordqry  ="select  collegeRollno,semester,stream,subject,subjectcode,	isnull(CoreCourse1,'') as CoreCourse1,	isnull(CoreCourse2,'') CoreCourse2,	isnull(CoreCourse3,'') as CoreCourse3,	isnull(generalCourse,'') as generalCourse,isnull(LCC,'') as LCC,isnull(DSE1,'') as DSE1,isnull(DSE2,'') as DSE2, isnull(DSE3,'') as DSE3,isnull(AECC,'') as AECC,isnull(SEC,'') as SEC from studentSemesterCourse where semester='".$SemforChange."' and collegeRollno='".$collegerollno."'";
 
	$result = $dbConn->query($recordqry);

	if($result) {
		while ($recordrow=$result->fetch(PDO::FETCH_ASSOC)){
 ...
 $courserecord = array();
    ...
    $courseqry	=	"select  collegeRollno,semester,	stream,	subject,subjectcode,	isnull(CoreCourse1,'') as CoreCourse1,	isnull(CoreCourse2,'') CoreCourse2,	isnull(CoreCourse3,'') as CoreCourse3,	isnull(generalCourse,'') as generalCourse,isnull(LCC,'') as LCC,isnull(DSE1,'') as DSE1,isnull(DSE2,'') as DSE2, isnull(DSE3,'') as DSE3,isnull(AECC,'') as AECC,isnull(SEC,'') as SEC from studentSemesterCourse where  collegeRollNo='".$collegerollno."' and semester='".$SemforChange."';";
        	...
        	<div class="row">
            	<div class="col-md-12">
                    <h5 class="text-danger border-bottom mb-3 pb-2">
                    	<i class="fa fa-search"></i> Update Course Detail (Semester <?php echo $SemforChange;?>)
                    </h5>
                </div>
				<div class="col-md-12">
				...
				<table class="table table-bordered table-striped shadow">
						...
						<?php foreach($headerarr as $headkey=>$headlabel) { ?>
                            <tr>
                                ...
                                <td><select id="cho_<?php echo $headkey;?>" name="cho_<?php echo $headkey;?>">
									<option value="<?php echo $courserecord[0][$headkey];?>"><?php echo $courserecord[0][$headkey];?></option>
									<select>
								</td>
                            </tr>
						<?php }?>
						...
						<tr>
                                <td colspan="2"><input type="button" id="btnupdate" name="btnupdate" onclick="updatecourseBuSem()" value="update" /></td>
