...
$coursestr ="";
	...
	if($_POST["choadmitsem"]!="All" ){
		$coursestr .=" and semester='".$_POST["choadmitsem"]."'";
   ...
   $qry = "select  spd.session as academicsession,st.collegeRollno,st.dob,spd.collegeRollNo,	spd.RollNo,spd.semester,isnull(spd.admissionToSemester,'') as admissionToSemester,
streamDisplay,subjectName , st.name, isNULL(st.CuFormNo,'') as CuFormNo, isNULL(st.CuRegNo,'') as CuRegNo, isNULL(st.CuRollNo,'') as CuRollNo, isnull(spd.applcntMobNo,'') as applcntMobNo,	isnull(spd.applcntEmail,'') as applcntEmail,ISNULL(spd.onlineTransactionID,'') as onlineTransactionID,isnull(spd.onlineTransactionDate,'') as onlineTransactionDate,isnull(spd.paymentdate,'') as paymentdate,
...
if($result) {
	...
	$record[] =$row; 
	$semesterarr[] = $row["admissionToSemester"];
	}
}
 

 $headqry = "select stream,semester,ltrim(rtrim(CoreCourse1)) as CoreCourse1,ltrim(rtrim(CoreCourse2)) CoreCourse2,	ltrim(rtrim(CoreCourse3)) as CoreCourse3,ltrim(rtrim(CoreCourse4)) as CoreCourse4,isnull(minorCourse2,'') as minorCourse2, isnull(IDC,'') as IDC,isnull(AECC,'') as AECC,isnull(SEC,'') as SEC,isnull(CVAC1,'') as CVAC1, isnull(CVAC2,'') as CVAC2 from Master_SemesterCourse WHERE stream='".$_POST["chostream"]."' ;";
...
cd.semester,cd.CoreCourse1,cd.CoreCourse2,cd.CoreCourse3,
cd.CoreCourse4,cd.minorCourse1,cd.minorCourse2,cd.IDC,cd.AECC,cd.SEC,cd.CVAC1,cd.CVAC2
  from studentmaster st LEFT JOIN studentSemesterCourse cd ON cd.collegeRollno=st.collegeRollno WHERE ISNULL(studentActiveStatus,0)=1 ".$coursestr."  and cd.semester='".$_POST["choadmitsem"]."' ;";
...
if($courseresult) {
 ...
 $semarr = array_values(array_unique($semesterarr));
...
$str = "";
 $str .= "<table class='table table-bordered table-hover'>";
				...
				$str .= "<td>".$record[$i]["admissionToSemester"]."</td>";
				foreach($semarr as $sem) {
					foreach($headrecord[$sem] as $headrec) {
					$str .="<td>".$sem."</td>";			
