...
// print_r($_POST);
 ...
 if(isset($_POST["content_id"])){
	...
	$getStudyMaterialQry = $dbConn->prepare("select * FROM LMS_study_material  where study_id= $study_id");
    $getStudyMaterialQry->execute();
    $getStudyMaterialRecord = $getStudyMaterialQry->fetch(PDO::FETCH_ASSOC);

	$paper_type_id = $getStudyMaterialRecord["paper_type_id"];
	$getPaperTypeQry = $dbConn->prepare("select * FROM LMS_paper_type_master  where paper_type_id= $paper_type_id");
    $getPaperTypeQry->execute();
    $getPaperTypeRecord = $getPaperTypeQry->fetch(PDO::FETCH_ASSOC);

	$course_id = $getStudyMaterialRecord["course_id"];
    $stream_id = $getStudyMaterialRecord["stream_id"];
    $department_id = $getStudyMaterialRecord["department_id"];
	$paper_duration = $getPaperTypeRecord["duration"];
