...
// print_r($_POST);
 ...
 if(isset($_POST["content_id"])){
	...
	$contentQry = $dbConn->prepare("select sc.content_id,sc.title,sc.content_type,sc.video_link,sc.document_path,sc.is_active,sc.publish_date,sm.course_id,sm.stream_id,sm.department_id,sm.material_id,sm.paper_type_id,sm.semester_id,sm.study_id,cm.course_name,dm.department_name,mm.material_name,ptm.paper_type_name FROM LMS_study_content as sc left join LMS_study_material as sm on sm.study_id = sc.study_id left join LMS_course_master as cm on sm.course_id = cm.course_id  left join LMS_department_master as dm on dm.department_id = sm.department_id
	...
	$getAllContentQry = $dbConn->prepare("select * FROM LMS_study_content  where study_id= $study_id");
