...
include("../../../configuration.php");
 ...
 if($printoption == 'Individual') {
	...
	$collegeRollNo = $_POST["txtCollegeRollNo"];
	    ...
	    if($semester=='I') {
	  $query = "select appliedsession,st.collegeRollNo,st.presentsemester as semester,st.applicationNo, name,appstream,appsubject,appsubjectcode,guardianName,st.applcntMobNo,bloodGroup, dd ,mm,yy,UPPER(localAddr)+', P.O-'+ UPPER(localPO)+', P.S-'+UPPER(localPS)+', City-'+UPPER(localCity)+',   Dist-'+UPPER(localDistrict)  as presentAddress,localPin as presentPin,photo from studentmaster st WHERE collegeRollNo='".$collegeRollNo."';";
		}
		else {
		$query = "select appliedsession,st.collegeRollNo,st.applicationNo, name,appstream,sp.semester,appsubject,appsubjectcode,guardianName,st.applcntMobNo,bloodGroup, dd ,mm,yy,UPPER(localAddr)+', P.O-'+ UPPER(localPO)+', P.S-'+UPPER(localPS)+', City-'+UPPER(localCity)+',   Dist-'+UPPER(localDistrict)  as presentAddress,localPin as presentPin,photo from studentmaster st JOIN studentPaymentDetail sp ON st.collegeRollno=sp.collegeRollNo WHERE st.collegeRollno='".$collegeRollNo."' and semester='".$semester."' and session='".$session."' and isnull(sp.Paid,'')='Y'  and FeeCode='ADMFEE';";	
		}
	   ...
	   if($_POST["chosubject"]!='All'){
		 $searchstr .= " and st.appsubjectcode='".$_POST["chosubject"]."'";
		  ...
		  if($semester=='I') {
			$query = "select appliedsession,st.collegeRollNo,st.presentsemester as semester,st.applicationNo, name,appstream,appsubject,appsubjectcode,guardianName,st.applcntMobNo,bloodGroup, dd ,mm,yy,UPPER(localAddr)+', P.O-'+ UPPER(localPO)+', P.S-'+UPPER(localPS)+', City-'+UPPER(localCity)+',   Dist-'+UPPER(localDistrict) as presentAddress,localPin as presentPin,photo from studentmaster st JOIN studentPaymentDetail sp ON st.collegeRollno=sp.collegeRollNo ".$searchstr."  and st.presentsemester='I' and  st.presentsession='".$session."' ORDER BY appstream asc,appsubjectcode,collegeRollNo asc, photo asc ;";	
		  }
		  else {
			 $query = "select appliedsession,st.collegeRollNo,st.applicationNo, sp.semester,name,appstream,appsubject,appsubjectcode,guardianName,st.applcntMobNo,bloodGroup, dd ,mm,yy,UPPER(localAddr)+', P.O-'+ UPPER(localPO)+', P.S-'+UPPER(localPS)+', City-'+UPPER(localCity)+',   Dist-'+UPPER(localDistrict)  as presentAddress,localPin as presentPin,photo from studentmaster st JOIN studentPaymentDetail sp ON st.collegeRollno=sp.collegeRollNo ".$searchstr." and semester='".$semester."' and session='".$session."' and isnull(sp.Paid,'')='Y' and FeeCode='ADMFEE' ORDER BY appstream asc,appsubjectcode,collegeRollNo asc, photo asc ;"; 
		  }
...
<link rel="stylesheet" type="text/css" href="id_css.css" media="print, screen" />
</head>
  ...
  <tr>
    <td style="padding:2px 0 0 0" valign="top" align="center">
        ...
        <div style="text-align:center; margin-top:5px">
            ...
            <img src="barcode.php?text=<?php echo $recordarr[$i]["collegeRollNo"];?>" width="55" height="25" style="margin-top:5px" />
        </div>
    </td>
    <td style="padding:3px 0 0 3px" valign="top">
        ...
        <div style="text-transform:uppercase; font-weight:bold; text-align:center; color:#c00; font-size:10px"><strong><?php echo displaySemester($recordarr[$i]["semester"]);?> Semester (<?php echo ACADEMIC_SESSION;?>)</strong></div>
        ...
        <td>
            ...
            Address: <strong style="font-size:10px"><?php echo $recordarr[$i]["presentAddress"].', '.$recordarr[$i]["presentPin"];?></strong></div>
            </td>
            </tr>
            </table>
