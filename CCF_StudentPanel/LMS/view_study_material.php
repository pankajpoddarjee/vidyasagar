...
$student_stream_code = isset($_SESSION["student_stream"])?$_SESSION["student_stream"]:'';
...
if($student_stream_code !== 'BCOM' && $student_stream_code !== 'BCOMGEN' && $student_stream_code !== 'BBA'){
    ...
    $streamSql = "select stream_id from LMS_stream_master where stream_code = '".$student_stream_code."' and is_active=1";
    $streamQry = $dbConn->prepare($streamSql);
    $streamQry->execute();
    $streamRecord = $streamQry->fetch(PDO::FETCH_ASSOC);
    $student_stream = $streamRecord['stream_id'];
...
left join CU_Master_Subject_Code_Type  as msc on msc.subject_id = sm.subject_id where sc.is_active=1 and CONCAT(',',sm.stream_id,',') LIKE CONCAT('%,','".$student_stream."',',%') ";
    ...
    $contentSql = "select sc.content_id,sc.title,sc.content_type,sc.video_link,sc.document_path,sc.is_active,sc.publish_date,tm.teacher_name,sc.created_at,cm.course_name,dm.department_name,mm.material_name,sm.semester_id,sm.subject_id FROM LMS_study_content as sc left join LMS_study_material as sm on sm.study_id = sc.study_id left join LMS_course_master as cm on sm.course_id = cm.course_id 
    left join LMS_department_master as dm on dm.department_id = sm.department_id
    left join LMS_material_master as mm on mm.material_id = sm.material_id
    left join LMS_teacher_master as tm on tm.teacher_id = sc.created_by
     where sc.is_active=1 and CONCAT(',',sm.stream_id,',') LIKE CONCAT('%,','".$student_stream."',',%') ";
                        ...
                        <table class="table table-bordered text-nowrap" id="tablefilter">
                            <thead class="text-center bg-light">
                                <tr>
                                    ...
                                    <th>Material Type</th>
                                    <?php if($student_stream_code !== 'BCOM' && $student_stream_code !== 'BCOMGEN' && $student_stream_code !== 'BBA'){ ?>
                                    <th>Paper Name</th>
                                    <th>Paper Type</th>
                                    <?php } ?>
                                    <th>Teacher</th>
                            ...
                            <tbody>
                               ...
                               <?php 
                                ...
                                <?php } } ?>
                                <?php 
                                if((isset($_POST["filter_sem"]) && $_POST["filter_sem"] == 'IV') || ($presentsemester=='IV' && !isset($_POST["filter_sem"]))){
                                ...
                                <?php 
                                if((isset($_POST["filter_sem"]) && $_POST["filter_sem"] == 'V') || ($presentsemester=='V' && !isset($_POST["filter_sem"]))){
                                ...
                                <?php 
                                if((isset($_POST["filter_sem"]) && $_POST["filter_sem"] == 'VI') || ($presentsemester=='VI' && !isset($_POST["filter_sem"]))){
                                ...
                                <?php 
                               if((isset($_POST["filter_sem"]) && $_POST["filter_sem"] == 'VII') || ($presentsemester=='VII' && !isset($_POST["filter_sem"]))){
                                ...
                                <?php 
                                if((isset($_POST["filter_sem"]) && $_POST["filter_sem"] == 'VIII') || ($presentsemester=='VIII' && !isset($_POST["filter_sem"]))){
