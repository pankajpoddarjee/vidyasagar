...
if($qryresult) {
 ...
 $feeqry ="select * from College_FeeStructureMaster where feeCode='".$studrecord[0]["FeeCode"]."' and semester='".$studrecord[0]["semester"]."' and feeGroup='Total';";
...
if($feeresult) {
  ...
  $str =BILLERID.'|'.$rollno.'|'.$studrecord[0]["id"].'|'.$feerecord[0]["FeeAmount"].'|NA|NA|NA|INR|NA|R|'.SECURITYID.'|NA|NA|F|'.$studrecord[0]["name"].'|'.$studrecord[0]["department"].'|'.$studrecord[0]["FeeCode"].'|NA|'.ACADEMIC_SESSION.'|'.$semester.'|'.$studrecord[0]["id"].'|'.BASE_URL_STUDENT.'/Otherpaymentsuccess.php';
